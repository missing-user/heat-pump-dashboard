...
def import_keymark_data(i=0):
    ...
    manufacturers_info = BeautifulSoup(
        requests.get("https://www.heatpumpkeymark.com/?type=109126").content,
        "html.parser",
    )
    ...
    for manufacturer_info in manufacturers_info.find_all("td")[i:]:
        ...
        print(
            ...
            i,
            " / ",
            len(manufacturers_info.find_all("td")),
            " In case of error: restart function with ",
            i - 1,
            " as input of function",
        )
        manufacturer = manufacturer_info.text
        models_info = BeautifulSoup(
            requests.get(
                "https://www.heatpumpkeymark.com/" + manufacturer_info.a.get("href")
            ).content,
            "html.parser",
        )
        # look for models:
        for model in models_info.find_all("td"):
            if model.text != manufacturer:
                ...
                if (
                    ...
                    try:
                        model_info = BeautifulSoup(
                            requests.get(
                                "https://www.heatpumpkeymark.com/" + model.a.get("href")
                            ).content,
                            "html.parser",
                        )
                    except:
                    ...
                    for info_col in model_info.find_all(class_="info-coll"):
                        if (info_col.find(class_="info-label").span.text) == (
                            "Refrigerant"
                        ):
                            ref = (
                                info_col.find(class_="info-data")
                                .text.replace(" ", "")
                                .replace("\n", "")
                                .replace("\r", "")
                            )
    ...
    with os.scandir(THIS_FOLDER_PATH + "/../input/" + foldername) as dir1:
        for file in dir1:
            ...
            try:
                ...
                continue
            try:
                df.loc[df["varName"] == "application", "value"] = df.loc[
                    df["varName"] == "Type", "value"
                ].values[0]
            except:
                print(file)
                ...
                for temperature in ["low", "high"]:
                    ...
                    for value, lst in zip(values, lists):
                        if value in general_info:
                            read_value = df.loc[df["varName"] == value, "value"].values[
                                0
                            ]
                        elif (value == "title") or (value == "application"):
                            read_value = df1.loc[
                                df1["varName"] == value, "value"
                            ].values[0]
                        else:
                            try:
                                read_value = df2.loc[
                                    df2["varName"] == value, "value"
                                ].values[0]
                            except:
                                read_value = ""
...
def reduce_heating_data():
    ...
    df.sort_values(
        by=["manufacturers", "models"], inplace=True, key=lambda col: col.str.lower()
    )
    df = df.loc[df["temperatures"] == "low"].merge(
        df.loc[df["temperatures"] == "high"], on="titels"
    )
    df = df[
        [
            "manufacturers_x",
    ...
    df.rename(
        columns={
            "manufacturers_x": "manufacturers",
            "models_x": "models",
            "titels": "titel",
            "dates_x": "dates",
            "types_x": "types",
            "refrigerants_x": "refrigerants",
            "mass_of_refrigerants_x": "mass_of_refrigerants",
            "spl_indoor_x": "spl_indoor_l",
            "spl_outdoor_x": "spl_outdoor_l",
            "eta_x": "eta_l",
            "p_rated_x": "p_rated_l",
            "scop_x": "scop_l",
            "t_biv_x": "t_biv",
            "tol_x": "tol",
            "p_th_minus7_x": "p_th_minus7_l",
            "cop_minus7_x": "cop_minus7_l",
            "p_th_2_x": "p_th_2_l",
            "cop_2_x": "cop_2_l",
            "p_th_7_x": "p_th_7_l",
            "cop_7_x": "cop_7_l",
            "p_th_12_x": "p_th_12_l",
            "cop_12_x": "cop_12_l",
            "p_th_tbiv_x": "p_th_tbiv_l",
            "cop_tbiv_x": "cop_tbiv_l",
            "p_th_tol_x": "p_th_tol_l",
            "cop_tol_x": "cop_tol_l",
            "rated_airflows_x": "rated_airflows_l",
            "wtols_x": "wtols",
            "poffs_x": "poffs",
            "ptos_x": "ptos",
            "psbs_x": "psbs",
            "pcks_x": "pcks",
            "supp_energy_types_x": "supp_energy_types",
            "p_sups_x": "p_sups_l",
            "p_design_cools_x": "p_design_cools_l",
            "seers_x": "seers_l",
            "pdcs_35_x": "pdcs_35_l",
            "eer_35_x": "eer_35_l",
            "pdcs_30_x": "pdcs_30_l",
            ...
            "pdcs_20_x": "pdcs_20_l",
            "eer_20_x": "eer_20_l",
            "spl_indoor_y": "spl_indoor_h",
            "spl_outdoor_y": "spl_outdoor_h",
            "eta_y": "eta_h",
            "p_rated_y": "p_rated_h",
            "scop_y": "scop_h",
            "p_th_minus7_y": "p_th_minus7_h",
            "cop_minus7_y": "cop_minus7_h",
            "p_th_2_y": "p_th_2_h",
            "cop_2_y": "cop_2_h",
            "p_th_7_y": "p_th_7_h",
            "cop_7_y": "cop_7_h",
            "p_th_12_y": "p_th_12_h",
            "cop_12_y": "cop_12_h",
            "p_th_tbiv_y": "p_th_tbiv_h",
            "cop_tbiv_y": "cop_tbiv_h",
            "p_th_tol_y": "p_th_tol_h",
            "cop_tol_y": "cop_tol_h",
            "rated_airflows_y": "rated_airflows_h",
            "p_sups_y": "p_sups_h",
            "p_design_cools_y": "p_design_cools_h",
            "seers_y": "seers_h",
            "pdcs_35_y": "pdcs_35_h",
            "eer_35_y": "eer_35_h",
            "pdcs_30_y": "pdcs_30_h",
...
def normalize_data():
    ...
    df.loc[(df["eer_20_l"].isna() == 0) & (df["p_el_35_h_n"].isna()), "eer_20_h"] = (
        df.loc[(df["eer_20_l"].isna() == 0) & (df["p_el_35_h_n"].isna()), "eer_20_l"]
        * (
            df[df["p_el_35_h_n"].isna() == 0]["eer_20_h"]
            / df[df["p_el_35_h_n"].isna() == 0]["eer_20_l"]
        )
        .unique()
        .mean()
    )
    df.loc[(df["eer_25_l"].isna() == 0) & (df["p_el_35_h_n"].isna()), "eer_25_h"] = (
        df.loc[(df["eer_25_l"].isna() == 0) & (df["p_el_35_h_n"].isna()), "eer_25_l"]
        * (
            df[df["p_el_35_h_n"].isna() == 0]["eer_25_h"]
            / df[df["p_el_35_h_n"].isna() == 0]["eer_25_l"]
        )
        .unique()
        .mean()
    )
    df.loc[(df["eer_30_l"].isna() == 0) & (df["p_el_35_h_n"].isna()), "eer_30_h"] = (
        df.loc[(df["eer_30_l"].isna() == 0) & (df["p_el_35_h_n"].isna()), "eer_30_l"]
        * (
            df[df["p_el_35_h_n"].isna() == 0]["eer_30_h"]
            / df[df["p_el_35_h_n"].isna() == 0]["eer_30_l"]
        )
        .unique()
        .mean()
    )
    df.loc[(df["eer_35_l"].isna() == 0) & (df["p_el_35_h_n"].isna()), "eer_35_h"] = (
        df.loc[(df["eer_35_l"].isna() == 0) & (df["p_el_35_h_n"].isna()), "eer_35_l"]
        * (
            df[df["p_el_35_h_n"].isna() == 0]["eer_35_h"]
            / df[df["p_el_35_h_n"].isna() == 0]["eer_35_l"]
        )
        .unique()
        .mean()
    )
    ...
    df.loc[
        (df["p_el_20_l_n"].isna() == 0) & (df["p_el_35_h_n"].isna()), "p_el_20_h_n"
    ] = (df[df["p_el_35_h_n"].isna() == 0]["p_el_20_h_n"].unique().mean())
    df.loc[
        (df["p_el_25_l_n"].isna() == 0) & (df["p_el_35_h_n"].isna()), "p_el_25_h_n"
    ] = (df[df["p_el_35_h_n"].isna() == 0]["p_el_25_h_n"].unique().mean())
    df.loc[
        (df["p_el_30_l_n"].isna() == 0) & (df["p_el_35_h_n"].isna()), "p_el_30_h_n"
    ] = (df[df["p_el_35_h_n"].isna() == 0]["p_el_30_h_n"].unique().mean())
    df.loc[
        (df["p_el_35_l_n"].isna() == 0) & (df["p_el_35_h_n"].isna()), "p_el_35_h_n"
    ] = (df[df["p_el_35_h_n"].isna() == 0]["p_el_35_h_n"].unique().mean())
    df.to_csv(
        r"../output/database_reduced_normalized.csv", encoding="utf-8", index=False
    )
...
def identify_subtypes():
    ...
    df["Subtype"] = np.where(
        (df["p_th_minus7_l"] <= df["p_th_2_l"])
        & (df["p_th_2_l"] <= df["p_th_7_l"])
        & (df["p_th_7_l"] <= df["p_th_12_l"]),
        "On-Off",
        "Regulated",
    )
    ...
    filt1 = (df["types"] == "Brine/Water and Water/Water") & (
        df["Subtype"] == "Regulated"
    )
    df.loc[filt1, "Group"] = 2
    ...
    df.to_csv(
        r"../output/database_reduced_normalized_subtypes.csv",
        encoding="utf-8",
        index=False,
    )
...
def interpolate_t_out(t_in):
    ...
    if t_in >= 7:
        ...
        i = 1
    t_out_l = t_out_low[i + 1] - (t_out_low[i + 1] - t_out_low[i]) * (
        t_in - t_amb[i + 1]
    ) / (t_amb[i] - t_amb[i + 1])
    t_out_h = t_out_high[i + 1] - (t_out_high[i + 1] - t_out_high[i]) * (
        t_in - t_amb[i + 1]
    ) / (t_amb[i] - t_amb[i + 1])
...
def calculate_fitting_parameters():
    ...
    for model in df["titel"]:
        ...
        if group == 1 or group == 2 or group == 3:
            COP_para_key = fit_simple(
                df_model["T_in"], df_model["T_out"], df_model["T_amb"], df_model["COP"]
            )
            df_model = df_model.loc[
                ((df_model["T_amb"] != 12) & (df_model["T_amb"] != 7))
            ]
            P_el_n_para_key = fit_simple(
                df_model["T_in"], df_model["T_out"], df_model["T_amb"], df_model["P_el"]
            )
        ...
        if df.loc[df["titel"] == (model), "seers_l"].isna().values[0] == 0:
            ...
            df_model["P_el"] = [
                (df.loc[df["titel"] == (model), "p_el_20_l_n"].values[0]),
            ...
            df_model["EER"] = [
                (df.loc[df["titel"] == (model), "eer_20_l"].values[0]),
            ...
            EER_para_key = fit_simple(
                df_model["T_in"], df_model["T_out"], df_model["T_amb"], df_model["EER"]
            )
            df_model = df_model.loc[df_model["T_in"] > 20]
            P_el_n_para_key_c = fit_simple(
                df_model["T_in"], df_model["T_out"], df_model["T_amb"], df_model["P_el"]
            )
    ...
    df["p4_EER [-]"] = p4_EER
    df.to_csv(
        r"../output/database_reduced_normalized_subtypes_parameters.csv",
        encoding="utf-8",
        index=False,
    )
    df.rename(
...
def validation_re_mape():
    ...
    while i < len(df):
        ...
        if group == 1 or group == 4:  # Air/Water
            ...
            df_model["T_in"] = [10, 10, 10, 10, 10, 10, 10, 10]
        res = heatpump.simulate(
            t_in_primary=df_model["T_in"].values,
            t_in_secondary=df_model["T_out"].values - 5,
            t_amb=df_model["T_amb"].values,
        )
        ...
        mape_pth.append(
            (((res["P_th_real"] - res["P_th"]).abs()) / res["P_th_real"] * 100).mean()
        )
        mape_pel.append(
            (((res["P_el_real"] - res["P_el"]).abs()) / res["P_el_real"] * 100).mean()
        )
        mape_cop.append(
            (((res["COP_real"] - res["COP"]).abs()) / res["COP_real"] * 100).mean()
        )
        if (df.loc[df["titel"] == (model), "seers_l"].isna().values[0] == 0) & (
            group == 1
        ):
            ...
            df_model["T_amb"] = [20, 20, 25, 25, 30, 30, 35, 35]
            df_model["T_out"] = [7, 18, 7, 18, 7, 18, 7, 18]
            res_c = heatpump.simulate(
                t_in_primary=df_model["T_in"].values,
                t_in_secondary=df_model["T_out"].values + 5,
                t_amb=df_model["T_in"].values,
                mode=2,
            )
            res_c = pd.DataFrame(res_c)
            res_c["Pdc_real"] = [
                (df.loc[df["titel"] == (model), "pdcs_20_l"].values[0]),
            ...
            res_c["Pel_c_real"] = [
                (df.loc[df["titel"] == (model), "p_el_20_l"].values[0]),
            ...
            mape_pdc.append(
                (((res["Pdc_real"] + res["P_th"]).abs()) / res["Pdc_real"] * 100).mean()
            )
            mape_pel_c.append(
                (
                    ((res["Pel_c_real"] - res["P_el"]).abs()) / res["Pel_c_real"] * 100
                ).mean()
            )
            mape_eer.append(
                (((res["EER_real"] - res["EER"]).abs()) / res["EER_real"] * 100).mean()
            )
        else:
    ...
    df = pd.read_csv("data/hplib_database.csv")
        ...
        if i > 50:
            for j in percent:
                if j < 1:
                    ...
                    ).sort_values(["SCOP"], ascending=False)[
                        int(j * i) : int(i * percent[percent.index(j) + 1])
                    ]
                    para = (
                        df_generic[
                            [
...
def calculate_cooling_parameters():
    ...
    para.rename(
        columns={"P_el_cooling_ref": "P_el_c_ref [W]", "Pdc_ref": "P_th_c_ref [W]"},
        inplace=True,
    )
    para = para[
        [
...
def validation_mape_cooling():
    ...
    para = pd.read_csv("data/hplib_database.csv", delimiter=",")
