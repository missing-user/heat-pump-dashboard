...
def get_parameters(
    ...
    """
    df = pd.read_csv(cwd() + r"/data/hplib_database.csv", delimiter=",")
...
def fit_p_th_ref(
    t_in: int, t_out: int, group_id: int, p_th_set_point: int, model: str
) -> Any:
    """
...
def fit_func_p_th_ref(
    p_th: int, t_in: int, t_out: int, group_id: int, p_th_set_point: int, model: str
) -> int:
    """
...
def simulate(
    t_in_primary: Union[float, np.ndarray],
    t_in_secondary: Union[float, np.ndarray],
    parameters,
    t_amb: Union[float, np.ndarray],
    mode: int = 1,
    p_th_min: Union[float, np.ndarray] = 0,
) -> dict:
    """
    ...
    if group_id in (1, 2, 3):
        if mode == 1:
            ...
            p_el = p_el_ref * (
                p1_p_el_h * t_in + p2_p_el_h * t_out + p3_p_el_h + p4_p_el_h * t_amb
            )
            if group_id == 1:
            ...
            if isinstance(cop, np.ndarray):
                ...
                p_el = np.where(
                    (cop > 1) & (p_th < p_th_min) & (p_el_ref < p_th_min / cop),
                    p_el_ref + p_th_ref,
                    p_el,
                )
                p_th = np.where(
        ...
        if mode == 2:
            ...
            t_amb = t_in
            p_el = (
                p1_p_el_c * t_in + p2_p_el_c * t_out + p3_p_el_c + p4_p_el_c * t_amb
            ) * p_el_col_ref
            if isinstance(p_el, np.ndarray):
...
class HeatPump:
    ...
    def simulate(
        ...
        if mode == 2 and self.group_id > 1:
            raise ValueError(
                "Cooling is only possible with heat pumps of group id = 1."
            )
        ...
        if self.group_id in (1, 2, 3):
            if mode == 1:
                ...
                if isinstance(cop, np.ndarray):
                    ...
                    p_el = np.where(
                        (cop > 1)
                        & (p_th < p_th_min)
                        & (self.p_el_ref < p_th_min / cop),
                        self.p_el_ref + self.p_th_ref,
                        p_el,
                    )
                    p_th = np.where(
                        (cop > 1)
                        & (p_th < p_th_min)
                        & (self.p_el_ref < p_th_min / cop),
                        self.p_el_ref * cop + self.p_th_ref,
                        p_th,
                    )
                    # increase electrical power for compressor
                    p_el = np.where(
            ...
            if mode == 2:
                ...
                p_el = (
                    self.p1_p_el_c * t_in
                    + self.p2_p_el_c * t_out
                    + self.p3_p_el_c
                    + self.p4_p_el_c * t_amb
                ) * self.p_el_col_ref
                if isinstance(p_el, np.ndarray):
                    eer = np.where(p_el < 0, 0, eer)
                    p_el = np.where(p_el < 0, 0, p_el)
        ...
        # for subtype = On-Off
            ...
            if isinstance(cop, np.ndarray):
                p_el = np.where(
                    (cop > 1) & (p_th < p_th_min), p_el + self.p_th_ref, p_el
                )
                p_th = np.where(
                    (cop > 1) & (p_th < p_th_min), p_th + self.p_th_ref, p_th
                )
                p_el = np.where(cop <= 1, self.p_th_ref, p_el)
                p_th = np.where(cop <= 1, self.p_th_ref, p_th)
                cop = p_th / p_el
    ...
    def calc_heating_dist_temp(self, t_avg_d: float):
        ...
        if t_avg_d > self.t_inside_set:
            ...
            t_hf = self.t_hf_min + (
                (1 / self.f_hs_size)
                * (
                    (self.t_inside_set - t_avg_d)
                    / (self.t_inside_set - self.t_outside_min)
                )
