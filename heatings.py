...
def cop_carnot(df, degradation_coeff=.5):
    result_df = pd.DataFrame()
    result_df['COP heatpump'] = degradation_coeff * (273.15 + df.loc[:,'T_vorlauf [°C]']) / (df.loc[:,'T_vorlauf [°C]'] - df.loc[:, 'T_outside [°C]'])
    return result_df
...
def simulate_hp_inverse(df, model='Bosch Compress 3000 AWS-8 B'):
    ...
    t_amb = np.arange(t_amb_min,t_amb_max+1.)
    ...
    calc_time = end - start
    results_lookup.loc[:,'COP Carnot'] = .4 * (273.15 + results_lookup['T_out']) / (results_lookup['T_out'] - results_lookup['T_in'])
    ...
    for iter,row in df[['Q_dot_H [kW]','T_outside [°C]']].iterrows():
        ...
        interpolation_function = interp1d(temp_df['P_th'].values, temp_df['COP'].values, kind='linear')
...
def simulate_hp(df,model):
    ...
    if model != 'Carnot':
        ...
        results_df = cop_carnot(df,degradation_coeff=.4)
